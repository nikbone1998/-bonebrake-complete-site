const RELEASE = '11.0.0';
const BUILD = 'phase11-operational-platform';
...
export default async function handler(req, res) {
  ...
  const payload = {
    ...
    release_status: environment === 'production'
      ? (ok ? 'phase11_production_healthy' : 'phase11_production_degraded')
      : (ok ? 'phase11_preview_healthy' : 'phase11_preview_degraded'),
    environment,
    region: process.env.VERCEL_REGION || null,
    frontend_backend_synchronized: true,
    data_plane: dataPlane,
    lead_pipeline: 'first_party_persistent_with_provider_fallback',
    audit: 'live_structural_persisted_with_history',
    analytics: 'first_party_attribution_events',
    owner_operations: 'supabase_auth_and_rls',
