...
project_metrics as (
  select count(*) filter (where status not in ('complete','completed','cancelled'))::int as active,
...
incident_metrics as (
  select count(*) filter (where status in ('open','retrying','escalated') and severity='critical')::int as critical,
...
fulfillment as (
  select count(*) filter (where status='waiting_intake')::int as waiting_intake,
...
revisions as (
  select count(*) filter (where status in ('pending','approved','processing'))::int as open,
    count(*) filter (where status='processing')::int as processing,
    count(*) filter (where status='failed')::int as failed
  from project_revision_requests
),
domains as (
  select count(*) filter (where is_primary=true and status in ('pending','awaiting_dns'))::int as pending,
...
releases as (
  select count(*) filter (where status='release_ready')::int as release_ready,
...
retry_metrics as (
  select count(*) filter (where status='scheduled')::int as scheduled,
    ...
    count(*) filter (where status='succeeded' and completed_at>=(select since_24h from p))::int as succeeded_24h
  from automation_retry_jobs
),
retry_attempt_metrics as (
  select count(*) filter (where status='failed' and completed_at>=(select since_24h from p))::int as failed_attempts_24h,
    count(*) filter (where status='succeeded' and completed_at>=(select since_24h from p))::int as succeeded_attempts_24h
  from automation_retry_attempts
),
dead_letter_metrics as (
  select count(*) filter (where status='open')::int as open,
    count(*) filter (where status='resolved' and resolved_at>=(select since_24h from p))::int as resolved_24h
  from automation_dead_letters
),
monitor as (
  select coalesce((select jsonb_build_object('status',r.status,'started_at',r.started_at,'completed_at',r.completed_at,'checks_run',r.checks_run,'incidents_seen',r.incidents_seen,'recoveries_attempted',r.recoveries_attempted,'recoveries_succeeded',r.recoveries_succeeded,'escalations_created',r.escalations_created) from automation_monitor_runs r order by r.started_at desc limit 1),'{}'::jsonb) as last_run,
...
select jsonb_build_object(
  ...
  'retries',(select to_jsonb(retry_metrics) || to_jsonb(retry_attempt_metrics) || jsonb_build_object('open_dead_letters',(select open from dead_letter_metrics),'resolved_dead_letters_24h',(select resolved_24h from dead_letter_metrics)) from retry_metrics,retry_attempt_metrics),
