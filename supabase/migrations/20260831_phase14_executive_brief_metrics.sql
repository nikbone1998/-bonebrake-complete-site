...
domains as (
  select
    count(*) filter (where is_primary=true and status in ('pending','verification_required'))::int as pending,
    count(*) filter (where status='failed' or ssl_status='failed')::int as failed,
    count(*) filter (where is_primary=true and status='active' and ssl_status='ready')::int as active_ready
