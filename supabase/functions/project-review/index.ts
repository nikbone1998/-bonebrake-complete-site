...
Deno.serve(async(req:Request)=>{
  ...
  const [{data:project},{data:job}]=await Promise.all([
    db.from('projects').select('*').eq('id',artifact.project_id).maybeSingle(),
    db.from('project_fulfillment_jobs').select('*').eq('id',artifact.fulfillment_job_id).maybeSingle()
  ])
  ...
  const {data:release}=await db.from('project_release_candidates').select('*').eq('project_id',project.id).eq('artifact_id',artifact.id).maybeSingle()
  const {data:revisions}=await db.from('project_revision_requests').select('id,created_at,status,request_text,submitted_by').eq('project_id',project.id).eq('artifact_id',artifact.id).order('created_at',{ascending:false}).limit(20)
  const previewUrl=`${url}/functions/v1/project-preview?t=${encodeURIComponent(token)}`

  if(action==='read') return Response.json({ok:true,project:{client_name:project.client_name,project_type:project.project_type},artifact:{id:artifact.id,version:artifact.version,title:artifact.title,summary:artifact.summary,status:artifact.status},preview_url:previewUrl,qa:{passed:job.qa_report?.passed===true,report:job.qa_report||{}},release_status:release?.status||null,revisions:revisions||[]},{headers})
  ...
  if(action==='request_revision'){
    ...
    const {data:revision,error:revisionError}=await db.from('project_revision_requests').insert({project_id:project.id,fulfillment_job_id:job.id,artifact_id:artifact.id,submitted_by:'client',request_text:requestText,status:'pending',metadata:{source:'private_review'}}).select('id').single()
    ...
    if(!actionId){const {data:queued}=await db.from('automation_actions').insert({action_type:'apply_paid_project_revision',entity_type:'project',entity_id:project.id,title:`Apply client revision for ${project.client_name}`,summary:'Client submitted revision feedback on the private preview. Generate a new preview version and rerun QA; production remains blocked.',risk_level:'approval',status:'pending',proposed_by:'client_review',payload:{project_id:project.id,fulfillment_job_id:job.id,artifact_id:artifact.id,revision_id:revision.id,external_effect:'preview_revision_only',production_release_authorized:false}}).select('id').single();actionId=queued?.id||null}
    await db.from('activity').insert({entity_type:'project',entity_id:project.id,action:'client_revision_requested',detail:{revision_id:revision.id,artifact_id:artifact.id,automation_action_id:actionId}})
    return Response.json({ok:true,revision_id:revision.id,action_id:actionId,status:'pending_owner_approval'},{headers})
  }

  const openRevisions=(revisions||[]).filter((r:any)=>['pending','approved','processing'].includes(r.status))
  if(openRevisions.length) return Response.json({ok:false,error:'revision_pending'},{status:409,headers})
  ...
  const releasePayload={project_id:project.id,fulfillment_job_id:job.id,artifact_id:artifact.id,status:'client_approved',qa_passed:true,qa_report:job.qa_report||{},payment_verified_at:approvedAt,client_approved_at:approvedAt,updated_at:approvedAt,metadata:{preview_version:artifact.version}}
  ...
  await db.from('activity').insert({entity_type:'project',entity_id:project.id,action:'client_preview_approved',detail:{artifact_id:artifact.id,release_candidate_id:candidate.id,release_action_id:releaseActionId}})
  return Response.json({ok:true,approved:true,release_candidate_id:candidate.id,owner_action_id:releaseActionId,production_deployed:false},{headers})
})