...
Deno.serve(async(req:Request)=>{
  ...
  const escalate=async(job:any,action:any,reason:string,errorMessage:string)=>{
    ...
    const stamp=now(),incidentKey=`automation:failed:${action.id}`,severity=action.action_type==='deploy_paid_project_production'?'critical':'error'
    const incidentPayload={action_type:action.action_type,error:clean(errorMessage,2000),retry_job_id:job.id,dead_letter_reason:reason}
    const{data:existingIncident}=await db.from('automation_incidents').select('id').eq('incident_key',incidentKey).maybeSingle()
    if(existingIncident){await db.from('automation_incidents').update({status:'escalated',category:'automation',severity,title:`Automation failed: ${action.title}`,detail:clean(errorMessage,4000),entity_type:'automation_action',entity_id:action.id,auto_recoverable:false,recovery_strategy:'escalate',last_seen_at:stamp,updated_at:stamp,last_payload:incidentPayload,escalation_action_id:escalationId,resolved_at:null,resolved_reason:null}).eq('id',existingIncident.id)}
    else{await db.from('automation_incidents').insert({incident_key:incidentKey,category:'automation',severity,status:'escalated',title:`Automation failed: ${action.title}`,detail:clean(errorMessage,4000),entity_type:'automation_action',entity_id:action.id,auto_recoverable:false,recovery_strategy:'escalate',max_auto_attempts:0,last_payload:incidentPayload,escalation_action_id:escalationId})}
    await db.from('automation_dead_letters').update({escalation_action_id:escalationId,updated_at:stamp}).eq('retry_job_id',job.id)
    await db.from('automation_retry_jobs').update({status:job.max_attempts>0&&job.attempt_count>=job.max_attempts?'exhausted':'manual',dead_lettered_at:stamp,escalation_action_id:escalationId,last_error:clean(errorMessage,2000),updated_at:stamp}).eq('id',job.id)
    deadLettered++;return escalationId
  }
  ...
  for(const action of failedActions||[]){
    const policy:any=policyMap.get(action.action_type);if(!policy)continue
    const{data:existing}=await db.from('automation_retry_jobs').select('*').eq('action_id',action.id).maybeSingle();if(existing)continue
    const error=clean(action.error_message||action.result?.error||'execution_failed',2000),nonRetry=!policy.auto_retry||policy.max_attempts<=0||hasPattern(error,policy.non_retryable_error_patterns||[])||(!action.approved_at&&action.risk_level!=='auto')
    const delay=nextDelay(policy,1),nextAt=nonRetry?null:new Date(Date.now()+delay*1000).toISOString(),status=nonRetry?'manual':'scheduled'
    const{data:job}=await db.from('automation_retry_jobs').insert({action_id:action.id,action_type:action.action_type,status,attempt_count:0,max_attempts:Number(policy.max_attempts||0),next_attempt_at:nextAt,last_error:error}).select('*').single();if(!job)continue
    captured++
    if(nonRetry)await escalate(job,action,!policy.auto_retry?'policy_manual_only':!action.approved_at&&action.risk_level!=='auto'?'approval_missing':'non_retryable_error',error)
    else await db.from('automation_actions').update({status:'approved',execute_after:nextAt,error_message:null,updated_at:now()}).eq('id',action.id).eq('status','failed')
  }
  ...
  for(const job of jobs||[]){
    const policy:any=job.automation_retry_policies||policyMap.get(job.action_type)
    if(!policy||!policy.enabled||!policy.auto_retry){const{data:action}=await db.from('automation_actions').select('*').eq('id',job.action_id).maybeSingle();if(action)await escalate(job,action,'policy_manual_only',job.last_error||'retry policy disabled');continue}
    const{data:action}=await db.from('automation_actions').select('*').eq('id',job.action_id).maybeSingle()
    if(!action){await db.from('automation_retry_jobs').update({status:'cancelled',completed_at:now(),updated_at:now(),last_error:'action_missing'}).eq('id',job.id);continue}
    ...
    if(!['failed','approved'].includes(action.status)){await db.from('automation_retry_jobs').update({status:'waiting',next_attempt_at:new Date(Date.now()+5*60_000).toISOString(),updated_at:now(),last_error:`action_state_${action.status}`}).eq('id',job.id);waiting++;continue}
    ...
    if(after?.status==='failed'){
      const err=clean(after.error_message||requestError||responseBody?.error||'retry_failed',2000),fatal=hasPattern(err,policy.non_retryable_error_patterns||[])
      await db.from('automation_retry_attempts').update({status:'failed',completed_at:stamp,http_status:httpStatus||null,error_message:err,response:responseBody||{}}).eq('id',attempt?.id);failed++
      if(fatal||nextAttempt>=Number(job.max_attempts||0)){await db.from('automation_retry_jobs').update({last_http_status:httpStatus||null,last_response:responseBody||{},last_error:err,updated_at:stamp}).eq('id',job.id);await escalate({...job,attempt_count:nextAttempt},after,fatal?'non_retryable_error_after_attempt':'attempts_exhausted',err)}
      else{
        const delay=nextDelay(policy,nextAttempt+1),nextAt=new Date(Date.now()+delay*1000).toISOString()
        const{error:armError}=await db.from('automation_actions').update({status:'approved',execute_after:nextAt,error_message:null,updated_at:stamp}).eq('id',after.id).eq('status','failed')
        if(armError){await escalate({...job,attempt_count:nextAttempt},after,'retry_rearm_failed',err);continue}
        await db.from('automation_retry_jobs').update({status:'scheduled',next_attempt_at:nextAt,last_http_status:httpStatus||null,last_response:responseBody||{},last_error:err,updated_at:stamp}).eq('id',job.id)
      }
      continue
    }
    const uncertain=clean(requestError||responseBody?.error||`action_state_${after?.status||'unknown'}`,1200)
    await db.from('automation_retry_attempts').update({status:'skipped',completed_at:stamp,http_status:httpStatus||null,error_message:uncertain,response:responseBody||{}}).eq('id',attempt?.id)
    await db.from('automation_retry_jobs').update({status:'waiting',next_attempt_at:new Date(Date.now()+10*60_000).toISOString(),last_http_status:httpStatus||null,last_response:responseBody||{},last_error:uncertain,updated_at:stamp}).eq('id',job.id);waiting++
  }
