...
Deno.serve(async(req:Request)=>{
  ...
  await db.from('activity').insert({entity_type:'project',entity_id:project.id,action:'owner_release_approved',detail:{artifact_id:artifact.id,release_candidate_id:candidate.id,production_deploy_enabled:!!settings.production_deploy_enabled}})
  const result={project_id:project.id,artifact_id:artifact.id,release_candidate_id:candidate.id,release_status:'release_ready',production_deploy_enabled:!!settings.production_deploy_enabled,production_deployed:false,production_backend_certification_required:true};await db.from('automation_actions').update({status:'completed',result,executed_at:readyAt,updated_at:readyAt}).eq('id',action.id).eq('status','executing');return Response.json({ok:true,status:'completed',result},{headers:{'Cache-Control':'no-store'}})
})
