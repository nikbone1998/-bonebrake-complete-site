...
const RETRYABLE_ACTIONS=new Set(['run_prospect_audit','promote_prospect_to_crm','prepare_paid_project_build'])
...
Deno.serve(async(req:Request)=>{
  ...
  let owner=false
  if(token){const {data:{user}}=await db.auth.getUser(token);owner=String(user?.email||'').toLowerCase()===OWNER}
  ...
  if(retryWorker){
    if(!RETRYABLE_ACTIONS.has(action.action_type)||!uuidRe.test(retryJobId)) return Response.json({ok:false,error:'retry_context_invalid'},{status:403})
