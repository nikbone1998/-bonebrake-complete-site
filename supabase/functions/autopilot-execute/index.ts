...
Deno.serve(async(req:Request)=>{
  ...
  if(action.action_type==='start_paid_project_fulfillment'&&!settings.fulfillment_enabled)
    return Response.json({ok:false,error:'fulfillment_disabled'},{status:423})
  ...
  try{
    if(claimed.action_type==='run_prospect_audit'){
      ...
      const auditRes=await fetch(`${url}/functions/v1/audit-run`,{
        method:'POST',headers:{'Content-Type':'application/json','apikey':publishable},
      ...
      if(ready){
        ...
        if(!existing?.length){
          const {error:qError}=await db.from('automation_actions').insert({
            ...
            title:`Promote ${candidate.company_name} to sales lead`,summary:`Audit passed · combined score ${ready.combined_score} · Tier ${candidate.qualification_tier}`,
    ...
    if(claimed.action_type==='promote_prospect_to_crm'){
      ...
      if(!leadId){
        ...
        const {data:lead,error:leadError}=await db.from('leads').insert({
          name:candidate.contact_name||candidate.company_name,email,phone:candidate.phone||null,company:candidate.company_name,website:candidate.website||null,
          source:`prospecting:${candidate.source_system}`,status:'qualified',priority:combined>=80?'high':'normal',estimated_value:0,
    ...
    if(claimed.action_type==='start_paid_project_fulfillment'){
      ...
      if(!intake){
        intakeToken=randomToken()
        const tokenHash=await sha256(intakeToken)
        const expiresAt=new Date(Date.now()+7*24*60*60*1000).toISOString()
        const {data:newIntake,error:intakeError}=await db.from('client_intake_requests').insert({
          project_id:project.id,lead_id:project.lead_id||null,client_email:email,token_hash:tokenHash,status:'pending',expires_at:expiresAt,
          metadata:{source:'autopilot_executor',package_key:project.project_type||null}
        }).select('*').single()
        if(intakeError||!newIntake) throw new Error('intake_request_creation_failed')
        intake=newIntake
      }
      ...
      if(!job){
        const {data:newJob,error:jobError}=await db.from('project_fulfillment_jobs').insert({
          project_id:project.id,lead_id:project.lead_id||null,intake_request_id:intake.id,status:intake.status==='submitted'?'intake_ready':'waiting_intake',
          build_profile:'paid_client_v1',requirements_snapshot:{package_key:project.project_type||null,agreed_price:Number(project.agreed_price),paid_amount:Number(project.paid_amount)}
        }).select('*').single()
        if(jobError||!newJob) throw new Error('fulfillment_job_creation_failed')
        job=newJob
      }

      const checklist=[
        ['client_intake','Client intake completed','onboarding'],
        ['content_assets','Content and brand assets ready','content'],
        ['build_generation','Website build generated','build'],
        ['automated_qa','Automated QA passed','qa'],
        ['client_review','Client review approved','review'],
        ['production_release','Production release approved','release']
      ].map(([item_key,label,category])=>({project_id:project.id,item_key,label,category,status:'pending'}))
      ...
      const {error:projectUpdateError}=await db.from('projects').update({
        status:'active',content_status:intake.status==='submitted'?'in_progress':'waiting_client',current_milestone:intake.status==='submitted'?'intake_ready':'client_intake',
        next_action:intake.status==='submitted'?'prepare_fulfillment_build':'await_client_intake',updated_at:now()
      }).eq('id',project.id)
      ...
      return await complete({
        project_id:project.id,fulfillment_job_id:job.id,intake_request_id:intake.id,intake_status:intake.status,
        intake_token:intakeToken,intake_expires_at:intake.expires_at,next_stage:intake.status==='submitted'?'intake_ready':'waiting_client_intake',
