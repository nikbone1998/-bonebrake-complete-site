...
Deno.serve(async(req:Request)=>{
  ...
  const [metricsRes,actionsRes,incidentsRes,projectsRes,activityRes]=await Promise.all([
  ...
  const m:any=metricsRes.data||{},actions=actionsRes.data||[],incidents=(incidentsRes.data||[]).sort((a:any,b:any)=>(severityRank[a.severity]??9)-(severityRank[b.severity]??9)),projects=projectsRes.data||[]
  const priorities:any[]=[]
  for(const i of incidents.slice(0,4))priorities.push({kind:'incident',severity:i.severity,title:i.title,detail:i.detail||`${i.category} incident`,incident_id:i.id,action_id:i.escalation_action_id||null})
  ...
  if(!priorities.length)priorities.push({kind:'status',severity:'normal',title:'No owner intervention required',detail:'Monitoring is healthy and no approval-risk action is waiting.'})
  const critical=Number(m.incidents?.critical||0),errors=Number(m.incidents?.error||0),warnings=Number(m.incidents?.warning||0),pending=Number(m.approvals?.pending||0),operationalFailures=Number(m.fulfillment?.failed||0)+Number(m.revisions?.failed||0)+Number(m.domains?.failed||0)+Number(m.releases?.failed_24h||0)
  ...
  const payload={version:'phase14-executive-brief-v1',generated_at:new Date().toISOString(),business_date:m.business_date,timezone,attention_level:attentionLevel,headline,metrics:m,priorities,approvals:actions,incidents,projects:projects.filter((p:any)=>!['complete','completed','cancelled'].includes(String(p.status||''))).slice(0,12),recent_activity:activityRes.data||[],capabilities:{monitoring_enabled:!!settings.monitoring_enabled,auto_recovery_enabled:!!settings.auto_recovery_enabled,autopilot_enabled:!!settings.autopilot_enabled,payments_enabled:!!settings.payments_enabled,fulfillment_enabled:!!settings.fulfillment_enabled,production_deploy_enabled:!!settings.production_deploy_enabled}}
