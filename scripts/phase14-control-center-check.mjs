...
const req=['autopilot-control-center.html','autopilot-control-center.css','autopilot-control-center.js','autopilot-control-center-safety.js','autopilot-control-center-extensions.js','autopilot-control-center-live.js','sales-previews/outreach-sample-archive.js','supabase/migrations/20260901_phase14_autopilot_control_center.sql'];
for(const f of req)if(!fs.existsSync(f))throw new Error(`missing ${f}`);
const html=fs.readFileSync('autopilot-control-center.html','utf8'),js=fs.readFileSync('autopilot-control-center.js','utf8'),safety=fs.readFileSync('autopilot-control-center-safety.js','utf8'),ext=fs.readFileSync('autopilot-control-center-extensions.js','utf8'),live=fs.readFileSync('autopilot-control-center-live.js','utf8'),css=fs.readFileSync('autopilot-control-center.css','utf8'),sql=fs.readFileSync('supabase/migrations/20260901_phase14_autopilot_control_center.sql','utf8'),archive=fs.readFileSync('sales-previews/outreach-sample-archive.js','utf8');
...
for(const token of ['EMAIL OUTREACH CENTER','What prospects received','COMMAND QUEUE','View What They Saw','gmail_message_id'])if(!ext.includes(token))throw new Error(`control center extension missing ${token}`);
for(const token of ['GLOBAL SEARCH','Find anything in Autopilot','Gmail ID','pollForChanges','sessionStorage'])if(!live.includes(token))throw new Error(`control center live/search layer missing ${token}`);
...
if(!html.includes('autopilot-control-center-live.js'))throw new Error('live/search layer is not wired into Control Center HTML');
...
if(/sb_secret_|SUPABASE_SERVICE_ROLE_KEY|service_role|sk_live_|ghp_/i.test(html+js+safety+ext+live+css+archive))throw new Error('privileged credential marker found in client assets');
...
for(const file of ['autopilot-control-center.js','autopilot-control-center-safety.js','autopilot-control-center-extensions.js','autopilot-control-center-live.js','sales-previews/outreach-sample-archive.js']){const syntax=spawnSync(process.execPath,['--check',file],{encoding:'utf8'});if(syntax.status!==0)throw new Error(`${file} syntax failed: ${syntax.stderr}`)}
