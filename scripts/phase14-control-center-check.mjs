...
if(/sb_secret_|SUPABASE_SERVICE_ROLE_KEY|service_role|sk_live_|ghp_/i.test(html+js+css+archive))throw new Error('privileged credential marker found in client assets');
...
const syntax=spawnSync(process.execPath,['--check','autopilot-control-center.js'],{encoding:'utf8'});if(syntax.status!==0)throw new Error(`Control Center JS syntax failed: ${syntax.stderr}`);
const archiveSyntax=spawnSync(process.execPath,['--check','sales-previews/outreach-sample-archive.js'],{encoding:'utf8'});if(archiveSyntax.status!==0)throw new Error(`sample archive syntax failed: ${archiveSyntax.stderr}`);
