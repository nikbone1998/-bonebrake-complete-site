...
if(/sb_secret_|SUPABASE_SERVICE_ROLE_KEY|service_role|sk_live_|ghp_/i.test(html+js+safety+ext+css+archive))throw new Error('privileged credential marker found in client assets');
...
for(const file of ['autopilot-control-center.js','autopilot-control-center-safety.js','autopilot-control-center-extensions.js','sales-previews/outreach-sample-archive.js']){const syntax=spawnSync(process.execPath,['--check',file],{encoding:'utf8'});if(syntax.status!==0)throw new Error(`${file} syntax failed: ${syntax.stderr}`)}
